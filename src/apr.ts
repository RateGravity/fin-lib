 ...
 */
export function computeApr(inputs: ComputeAprInputs, computeMI: ComputeMI = computePMI): number {
  ...
  const paymentGroups = computeMI(inputs, loanAmortization)
    .map(
      // Include the mortgage insurance + feeAllocation + principal + interest in
      // the monthly payment
      ({ principal, interest, mortgageInsurance }, i) => {
        const feeAllocation =
          i < feeAmortization.length
            ? feeAmortization[i].principal + feeAmortization[i].interest
            : 0;
        return principal + interest + feeAllocation + mortgageInsurance;
      }
    )
    .reduce((grouped, monthlyPayment) => {
  ...
  let rate = inputs.initialRate / MonthlyRateDecimal;
  let diff = rate;
  for (let t = 0; t < 100; t++) {
  ...
  return rate * MonthlyRateDecimal;
}
